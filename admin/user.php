...
if (isset($_POST['tambah_user']) && csrf_validate()) {
    $name = trim($_POST['name'] ?? '');
    $email = trim($_POST['email'] ?? '');
    $password = $_POST['password'] ?? '';
    $role = $_POST['role'] ?? 'customer';
    if (!in_array($role, ['admin', 'customer'])) $role = 'customer';
    $exists = db_query_one($conn, "SELECT id FROM users WHERE email = ?", ["s", $email]);
    if ($exists) {
        flash('error', 'Email sudah terdaftar.');
    } else {
        db_insert($conn, "INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)", ["ssss", $name, $email, md5($password), $role]);
        flash('success', 'Pengguna berhasil ditambahkan!');
    }
    header("Location: user.php");
    exit;
}
if (isset($_POST['update_user']) && csrf_validate()) {
    ...
    $exists = db_query_one($conn, "SELECT id FROM users WHERE email = ? AND id != ?", ["si", $email, $id]);
    if ($exists) {
        flash('error', 'Email sudah digunakan pengguna lain.');
    } else {
        if ($password !== '') {
            db_execute($conn, "UPDATE users SET name = ?, email = ?, password = ?, role = ? WHERE id = ?", ["ssssi", $name, $email, md5($password), $role, $id]);
        } else {
            db_execute($conn, "UPDATE users SET name = ?, email = ?, role = ? WHERE id = ?", ["sssi", $name, $email, $role, $id]);
        }
...
?>
        ...
        <div class="flex items-center gap-4 mb-3">
            <form method="GET" action="" class="relative flex-1">
                ...
                <input type="text" name="q" value="<?= isset($_GET['q']) ? e($_GET['q']) : '' ?>" placeholder="Cari nama atau email pengguna..." class="w-full py-2 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-green-500 transition" style="padding-left: 2.5rem; padding-right: 2.5rem;">
                <?php if (isset($_GET['q']) && $_GET['q'] !== ''): ?>
                <a href="user.php<?= isset($_GET['role']) && $_GET['role'] !== '' ? '?role=' . e($_GET['role']) : '' ?>" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
        ...
        <?php
        $role_counts = [];
        $rows_role = db_query($conn, "SELECT role, COUNT(*) as total FROM users GROUP BY role", []);
        foreach ($rows_role as $rr) { $role_counts[$rr['role']] = (int) $rr['total']; }
        $total_users = array_sum($role_counts);
        $current_role = $_GET['role'] ?? '';
        $q_param = isset($_GET['q']) && $_GET['q'] !== '' ? '&q=' . urlencode($_GET['q']) : '';
        ?>
        <div class="flex items-center gap-2 mb-5 flex-wrap">
            <a href="?<?= ltrim($q_param, '&') ?>" class="text-xs font-bold px-3 py-1.5 rounded-lg transition <?= $current_role === '' ? 'ring-2 ring-offset-1 ring-gray-300' : 'hover:opacity-80' ?>" style="color:#374151; background:#E5E7EB">Semua <span class="ml-1 opacity-70"><?= $total_users ?></span></a>
            <a href="?role=admin<?= $q_param ?>" class="text-xs font-bold px-3 py-1.5 rounded-lg transition <?= $current_role === 'admin' ? 'ring-2 ring-offset-1' : 'hover:opacity-80' ?>" style="color:#1565C0; background:#E3F2FD; <?= $current_role === 'admin' ? 'ring-color:#1565C0' : '' ?>">Admin <span class="ml-1 opacity-70"><?= $role_counts['admin'] ?? 0 ?></span></a>
            <a href="?role=customer<?= $q_param ?>" class="text-xs font-bold px-3 py-1.5 rounded-lg transition <?= $current_role === 'customer' ? 'ring-2 ring-offset-1' : 'hover:opacity-80' ?>" style="color:#2E7D32; background:#E8F5E9; <?= $current_role === 'customer' ? 'ring-color:#2E7D32' : '' ?>">Customer <span class="ml-1 opacity-70"><?= $role_counts['customer'] ?? 0 ?></span></a>
        </div>
                ...
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-2xl pointer-events-auto modal-content" style="transform:scale(0.95);opacity:0;transition:transform 0.25s cubic-bezier(0.21,1.02,0.73,1),opacity 0.2s">
                    ...
                    <form method="POST" class="p-6 space-y-4">
                        ...
                        <div class="flex justify-end gap-2 pt-2">
                            ...
                            <button type="submit" name="tambah_user" class="px-5 py-2.5 text-white rounded-xl text-sm font-semibold hover:opacity-90 transition" style="background:#42B549">Tambah User</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-2xl pointer-events-auto modal-content" style="transform:scale(0.95);opacity:0;transition:transform 0.25s cubic-bezier(0.21,1.02,0.73,1),opacity 0.2s">
                    ...
                    <form method="POST" class="p-6 space-y-4">
                        ...
                        <div class="flex justify-end gap-2 pt-2">
                            ...
                            <button type="submit" name="update_user" class="px-5 py-2.5 text-white rounded-xl text-sm font-semibold hover:opacity-90 transition" style="background:#1976D2">Simpan Perubahan</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    $paging = paginate($conn, "SELECT COUNT(*) as c FROM users u" . $where, $params, 15);
                    $users = db_query($conn, "SELECT u.*, (SELECT COUNT(*) FROM transaksi WHERE user_id = u.id) as jumlah_transaksi FROM users u" . $where . " ORDER BY u.role ASC, u.name ASC LIMIT " . $paging['limit'] . " OFFSET " . $paging['offset'], $params);
                    if (count($users) == 0): ?>
                        ...
                        <td class="px-6 py-4 text-center">
                            <?php if($r['id'] != $_SESSION['id']){ ?>
                                ...
                                <form method="POST" class="inline" onsubmit="return confirm('Hapus user ini?')">
                                    ...
                                    <button type="submit" name="hapus_user" value="1" class="inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-lg transition cursor-pointer" style="background:#FFEBEE; color:#C62828">Hapus</button>
                                </form>
                            <?php } else { ?>
                                <button onclick="openEdit(<?= $r['id'] ?>, <?= htmlspecialchars(json_encode($r['name']), ENT_QUOTES) ?>, <?= htmlspecialchars(json_encode($r['email']), ENT_QUOTES) ?>, <?= htmlspecialchars(json_encode($r['role']), ENT_QUOTES) ?>)" class="inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-lg mr-1 transition cursor-pointer" style="background:#FFF8E1; color:#F57F17">Edit</button>
