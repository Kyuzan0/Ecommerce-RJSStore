...
<div class="ds-page-header">
    <h1 class="ds-page-title">Produk Digital</h1>

    <!-- Search Form -->
    <form method="GET" action="<?= url('/customer/produk') ?>" class="w-full sm:max-w-md mt-3">
        <div class="relative">
            <input type="text"
                   name="search"
                   value="<?= e($search) ?>"
                   placeholder="Cari produk..."
                   class="w-full px-4 py-2.5 pr-10 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent">
            <button type="submit" aria-label="Cari" class="absolute right-1 top-1/2 -translate-y-1/2 w-10 h-10 inline-flex items-center justify-center text-gray-400 hover:text-green-600">
...
<?php if (empty($products)): ?>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8 sm:p-12 text-center">
        ...
        <h3 class="text-base sm:text-lg font-semibold text-gray-700 mb-2">Tidak ada produk ditemukan</h3>
        <p class="text-sm text-gray-500">Coba kata kunci pencarian lain</p>
    </div>
<?php else: ?>
    <!-- Product Grid -->
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-8">
        <?php
        $tipe_chip_map = [
            'Aplikasi' => 'accent',
            'Game'     => 'warning',
        ...
        ?>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden product-card cursor-pointer flex flex-col" data-product-id="<?= $pid ?>" data-cart-state="<?= $is_purchased ? 'purchased' : ($is_in_cart ? 'in_cart' : 'default') ?>" onclick="openProductModal(<?= $pid ?>, event)">
            <!-- Product Image -->
            <div class="h-32 sm:h-40 lg:h-48 bg-gray-100 flex items-center justify-center">
                <svg class="w-10 h-10 sm:w-14 sm:h-14 lg:w-16 lg:h-16 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-3 sm:p-4 flex flex-col flex-1">
                <!-- Type Badge -->
                <span class="ds-chip ds-chip--<?= e($chip_variant) ?> mb-2 self-start">
                    <?= e($product['tipe_produk']) ?>
                </span>

                <!-- Product Name -->
                <h3 class="font-semibold text-gray-800 text-sm sm:text-base mb-2 line-clamp-2"><?= e($product['nama_produk']) ?></h3>

                <!-- Rating -->
                <div class="flex items-center mb-2">
                    <?php
                    $avg_rating = $product['avg_rating'] ?? 0;
                    $total_rating = $product['total_rating'] ?? $product['total_reviews'] ?? 0;
                    for ($i = 1; $i <= 5; $i++):
                    ?>
                        <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 <?= $i <= $avg_rating ? 'text-yellow-400' : 'text-gray-300' ?>" fill="currentColor" viewBox="0 0 20 20">
                ...
                <!-- Price -->
                <?php if ($is_akun): ?>
                    <p class="text-base sm:text-lg font-bold text-green-600 mb-3 mt-auto"><span class="text-xs font-normal text-gray-400">Mulai </span><?= rupiah($product['harga']) ?></p>
                <?php else: ?>
                    <p class="text-base sm:text-lg font-bold text-green-600 mb-3 mt-auto"><?= rupiah($product['harga']) ?></p>
                <?php endif; ?>

                <!-- Action Button -->
                <?php if ($is_purchased): ?>
                    <button disabled class="w-full bg-gray-300 text-gray-500 py-2 sm:py-2.5 px-3 rounded-xl text-xs sm:text-sm font-semibold cursor-not-allowed">
                        Sudah Dibeli
                    </button>
                <?php elseif ($is_in_cart): ?>
                    <button data-cart-produk="<?= $pid ?>" disabled class="w-full bg-orange-400 text-white py-2 sm:py-2.5 px-3 rounded-xl text-xs sm:text-sm font-semibold cursor-not-allowed">
                        Di Keranjang
                    </button>
                <?php elseif ($is_akun): ?>
                    <button onclick="openProductModal(<?= $pid ?>); event.stopPropagation();"
                            class="w-full bg-green-600 hover:bg-green-700 text-white py-2 sm:py-2.5 px-3 rounded-xl text-xs sm:text-sm font-semibold transition">
                        Pilih Varian
                    </button>
                <?php else: ?>
            ...
            <div id="mp-content" class="hidden h-full overflow-y-auto md:overflow-hidden md:grid md:grid-cols-2">
                <div class="relative flex flex-col min-h-[320px] bg-white border-b md:border-b-0 md:border-r border-white/60 overflow-y-visible md:overflow-y-auto" id="mp-hero-bg" style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%)">
                    <div class="relative z-10 flex-1 flex flex-col px-6 py-5 md:px-8 md:py-8">
                        <div class="mb-4 inline-flex items-center justify-center p-4 rounded-3xl bg-white/90 shadow self-start" id="mp-icon-container">
                        ...
                        <h2 id="mp-title" class="font-extrabold text-xl md:text-2xl text-gray-900 mb-3 leading-tight"></h2>
                ...
                <div class="flex flex-col bg-gradient-to-b from-white to-[#f7faf7] min-h-[320px]">
                    ...
                    <div class="flex-1 overflow-y-visible md:overflow-y-auto px-6 py-6 md:px-8 md:py-8" id="mp-body">
                        <div id="mp-reviews" class="grid grid-cols-1 gap-4 items-start min-h-[280px]"></div>
...
function renderReviewsPage() {
    ...
    pageItems.forEach(function(rev) {
        ...
        reviewsEl.innerHTML += '<div class="bg-gray-50 rounded-2xl p-5 border border-gray-100/50"><div class="flex items-center gap-3 mb-3"><div class="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold" style="background:#42B549">' + escapeHtml(rev.initial) + '</div><div><p class="text-sm font-bold text-gray-800">' + escapeHtml(rev.nama_user) + '</p><span class="text-xs text-gray-400">' + escapeHtml(rev.tanggal) + '</span></div><div class="flex items-center gap-0.5 ml-auto">' + stars + '</div></div><p class="text-sm text-gray-600">' + (rev.ulasan ? escapeHtml(rev.ulasan) : '<span class="text-gray-400 italic">Hanya memberikan rating</span>') + '</p></div>';
    });
