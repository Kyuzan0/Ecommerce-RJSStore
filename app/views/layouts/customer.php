...
<header class="sticky top-0 z-40 border-b">
    ...
    <div class="px-4 md:hidden pb-3">
        <form action="<?= url('/customer/produk') ?>" method="GET" class="flex bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
            ...
            <button type="submit" class="px-4 py-2.5 text-white text-sm font-semibold" style="background:#42B549">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            </button>
        </form>
    </div>
...
<div class="app-shell__body flex flex-1 lg:overflow-hidden">
    <aside class="app-shell__aside flex-shrink-0 flex flex-col pt-4 pb-6 px-3 overflow-y-auto border-r">
        ...
        <nav class="flex flex-col gap-1 flex-1" onclick="if(window.innerWidth<1024)document.body.classList.remove('aside-open')">
            ...
            <a href="<?= url('/customer/download') ?>" class="sidebar-link <?= ($active_page ?? '') === 'download' ? 'active' : '' ?>"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>Koleksi Saya</a>
    ...
    <main class="flex-1 flex flex-col p-4 md:p-6 lg:overflow-y-auto">
        <?= flash_render() ?>
        <?= $content ?>
    </main>
</div>
