    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center gap-3 md:gap-4 flex-wrap md:flex-nowrap">
        ...
        <div class="order-2 md:order-3 flex items-center gap-2 md:gap-3 ml-auto">
            ...
            <?php if ($this->auth->isCustomer()): ?>
                ...
                <div class="relative" id="profileDropdown">
                    ...
                    <div id="profileMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50">
                        <a href="<?= url('/customer/dashboard') ?>" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
            ...
            <?php elseif (!$this->auth->check()): ?>
                <?php if (!$_is_auth_page): ?>
                <button onclick="toggleDarkMode()" class="hidden sm:block p-2 rounded-lg hover:bg-gray-100 transition text-gray-500" title="Mode Gelap" aria-label="Toggle dark mode">
                ...
                <?php endif; ?>
                <a href="<?= url('/auth/login') ?>" class="text-sm font-semibold text-white px-4 py-2 rounded-lg transition hover:opacity-90" style="background:#42B549">Masuk</a>
            <?php endif; ?>
        </div>
    </div>
</header>
